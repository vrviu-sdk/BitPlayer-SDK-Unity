...
{
    /// <summary>
    /// This is core staff of video player, utilize it to control player statemachine trainsitions,
    /// for window platform use ffmpeg as default decoder, android platform use native MediaCodec,
    /// iOS use AVPlayer.
    /// </summary>
    public unsafe class BitPlayerTexture : MonoBehaviour
    {
#else
namespace VRVIU.BitVRPlayer.BitVideo
{
public class BitPlayerTexture : MonoBehaviour
{
#endif
	...
	Call_SetUnityTexture (m_VideoTexture.GetNativeTextureID ());
        ...
        void OnError(int iCode, int iCodeExtra)
        {
            string strError = "error type:";

            switch (iCode)
            {
                case ErrorCode.ERROR_SOURCE:
                    strError += "source";
                    break;
                case ErrorCode.ERROR_RENDERER:
                    strError += "renderer";
                    break;
                default:
                    strError += "unexpected";
            ...
            switch (iCodeExtra)
            {
                case ErrorCode.ERROR_MEDIA_SOURCE:
                    strError += "error_media_source";
                    break;
                case ErrorCode.ERROR_QUERYING_DECODERS:
                    strError += "error_querying_decoders";
                    break;
                case ErrorCode.ERROR_NO_SECURE_DECODER:
                    strError += "error_no_secure_decoder";
                    break;
                case ErrorCode.ERROR_NO_DECODER:
                    strError += "error_no_decoder";
                    break;
                case ErrorCode.ERROR_INSTANTIATING_DECODER:
                    strError += "error_instantiating_decoder";
                    break;
                default:
...
#else
            Call_init(account.appId, account.bizId, account.accessKeyId, account.accessKey, account.sdkVersion);
#endif
        }
	...
	{
	    OnError(ErrorCode.ERROR_UNEXPECTED, ErrorCode.ERROR_UNEXPECTED);
	    return false;
	}
	}
#else
	...
	{
	    OnError(EXOPLAYER_ERROR.ERROR_UNEXPECTED, EXOPLAYER_ERROR_EXTRA_CODE.ERROR_UNEXPECTED);
	    return false;
	}
#endif
	}
    ...
    private void Call_init(string appId,string bizzId,string accessKeyId,string accessKey,string sdkVersion){
        GetJavaObject().Call("init",appId,bizzId,accessKey,accessKeyId,sdkVersion);
    }
	...
	audioSource.time = fLastFrameTime;
                    ...
                    if (m_bSetTextureFirst == false)
                    {
                        ...
                        if (mShaderYUV == null)
                        {
                            mShaderYUV = Shader.Find("Unlit/Unlit_YUV");
                        }

                        if (material == null)
                            material = new Material(mShaderYUV);
