namespace VRVIU.BitVrPlayer.Silver
{
    ...
    public class SilverMediaPlayerCtrl : MonoBehaviour
    {
        ...
        public bool m_bEnableAutoRotation = true;
    ...
    { MEDIAPLAYER_STATE.PLAYING, MEDIAPLAYER_STATE.END, MEDIAPLAYER_STATE.READY, MEDIAPLAYER_STATE.STOPPED };
        ...
        void Start()
        {
            ...
            if (m_bDisplayRotation)
            {
                ...
                m_DebugObject.layer = LayerMask.NameToLayer("UI");
                ...
                m_DebugTextObject.fontSize = 15;
                m_DebugTextObject.color = Color.red;
            ...
            if (VIDEO_PLAYBACK_ENABLED)
            {
                ...
                if (m_CurrentState.eState == MEDIAPLAYER_STATE.PLAYING || m_bPause)
                {
                    ...
                    if (m_bEnableAutoRotation)
                    {
                        ...
                        Camera.main.transform.rotation = new Quaternion(0.0f, m_debugRotation, 0.0f, 0.0f);
                    ...
                    else
                    {
                        if (UnityEngine.XR.XRDevice.isPresent == false)
                        {
                            ...
                            m_headOrientation = Camera.main.transform.rotation.eulerAngles;
                            m_headOrientation.x = -m_headOrientation.x;             // Invert pitch *BW* not sure why
#else
                        // get orientation from device's gyro input
                        Camera.main.transform.rotation = ConvertRotation(Input.gyro.attitude);
#endif
                        }
                        else
                        {
                    ...
                    if (isFrameReady())
                    {
                        ...
                        m_CurrentState.lTimestamp = Call_UpdateVideoTexture(out m_viewOrientation);
                        if (m_bDisableOrientationSetting)
                        {
                        ...
                        {
                            setMaterialOrientation(currentOrientation);
                        }
                    ...
                    if (m_DebugTextObject != null)
                    {
                        ...
                        m_DebugTextObject.text = "Head orientation:" + m_headOrientation + "\n" +
                                                 ...
                                                 string.Format("Timestamp: {0,6:##0.000}", (double)m_CurrentState.lTimestamp / 1e6);
                    }
                }
                UpdateState();
            }
        ...
        private long Call_UpdateVideoTexture(out Vector3 orientation)
        {
            long lTimestamp = 0;
            GL.InvalidateState();
            ...
            if (VIDEO_PLAYBACK_ENABLED)
            {
                textureData = Call_GetVideoTexture(out lTimestamp, out orientation);
            }
            else
            {
        ...
        private IntPtr[] Call_GetVideoTexture(out Int64 lTimestamp, out Vector3 orientation)
        {
            IntPtr[] texture;
            m_Player.GetTexture(out texture, out lTimestamp, out orientation);
            return texture;
        }
