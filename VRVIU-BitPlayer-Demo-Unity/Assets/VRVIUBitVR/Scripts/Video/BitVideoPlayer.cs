      ...
      /*  public void Load() {
        ...
        */
        ...
        private void SwitchVideoFormat()
        {
            ...
            {
                mSilverPlayer.m_ViewerFormatScript = gameObject.AddComponent<SetViewerFormat>();
                switch ((Data.VideoFormat)mData.format)
                {
                    case Data.VideoFormat.Mono:
                        SetViewerFormat.m_formatType = "mono";
                        break;
                    case Data.VideoFormat.TopBottom:
                        SetViewerFormat.m_formatType = "stereo_top_bottom";
                        break;
                    case Data.VideoFormat.LeftRight:
            ...
            {
                if (mData.format == (int)Data.VideoFormat.Mono)
                {
                    if (mData.formatDegree == 180)
                    {
                        mVideoFormater.Switch(Video.VideoFormat.OPT_ERP_180);
                    }
                    else
                    {
                        mVideoFormater.Switch(Video.VideoFormat.OPT_ERP_360);
                    }
                }
                else if (mData.format == (int)Data.VideoFormat.LeftRight)
                {
                    if (mData.formatDegree == 180)
                    {
                        mVideoFormater.Switch(Video.VideoFormat.OPT_ERP_180_LR);
                    }
                    else
					{
                        mVideoFormater.Switch(Video.VideoFormat.OPT_ERP_360_LR);
                    }
                }
                else if (mData.format == (int)Data.VideoFormat.TopBottom)
                {
                    if (mData.formatDegree == 180)
                    {
                        mVideoFormater.Switch(Video.VideoFormat.OPT_ERP_360_LR);
                    ...
                    {
                        mVideoFormater.Switch(Video.VideoFormat.OPT_ERP_360_TB);
                    }
                }
            }
            else if (mData.meshType == (int)MeshType.Fisheye)
            {
                if (mData.format == (int)Data.VideoFormat.Mono)
                {
                    mVideoFormater.Switch(Video.VideoFormat.OPT_FISHEYE);
                }
                else if (mData.format == (int)Data.VideoFormat.LeftRight)
                {
                    mVideoFormater.Switch(Video.VideoFormat.OPT_FISHEYE_LR);
                }
                else if (mData.format == (int)Data.VideoFormat.TopBottom)
                {
                    mVideoFormater.Switch(Video.VideoFormat.OPT_FISHEYE_TB);
                }
            }
