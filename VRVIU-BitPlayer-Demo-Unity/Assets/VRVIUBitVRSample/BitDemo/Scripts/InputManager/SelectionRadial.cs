...
namespace VRVIU.LightVR.InputManager
{
    ...
    public class SelectionRadial : AbstractSelector
    {
        ...
        public override void Show()
        {
			...
			// will set reticle selection radial to trigger when user looks at selection
			HandleDown ();
        }
        ...
        private IEnumerator FillSelectionRadial()
        {
            ...
            // This loop is executed once per frame until the timer exceeds the duration (or the radial is set inactive if the user moves focus off a selection).
            while ((timer < m_SelectionDuration) && (m_IsSelectionRadialActive))
            {
            ...
            // if the user didn't move focus from the selection, so the selection radial is still active
            if (m_IsSelectionRadialActive)
            {
                // Set the fill amount to be full.
            ...
            {
                // Set the fill amount to be empty.
                m_Selection.fillAmount = 0f;
            }
        }
        ...
        public IEnumerator WaitForSelectionRadialToFill ()
        {
            ...
            // Check every frame if the radial is filled (and that the selection radial is still active i.e. user not moved focus away from selection).
            while ((!m_RadialFilled) && (m_IsSelectionRadialActive))
            {
                yield return null;
            }

            // Once it's been used make the radial invisible (unless selection radial is already inactive due to user moving focus away).
            if (m_IsSelectionRadialActive)
            {
                Hide();
            }
